...
import { editProductStore } from "@/redux/slices/productSlice";
import {
  setSelectedImage,
...
const AddProductDialogForm = ({ product }: { product: product }) => {
  ...
  async function onSubmit(values: z.infer<typeof FormSchema>) {
    ...
    const editedProduct = await handleImageUpload(
      selectedImage,
      response.data.productId
    );

    editedProduct
      ? dispatch(editProductStore(editedProduct))
      : dispatch(editProductStore(response.data));
          ...
          render={({ field }) => (
            <FormItem>
              ...
              <Select
                ...
                defaultValue={Number(product.productIsActive).toString()}
              >
                <FormControl>
                  <SelectTrigger>
